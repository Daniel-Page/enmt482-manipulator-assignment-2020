...
def axesrgb1(x=0,y=0,z=0,turn=0):
	x22, y22, z22 = cylinder(r2, -h2, a=a2)

	cyl_z = go.Surface(x=x+x2, y=y+y2, z=(z+z22),
					 colorscale = colorscale_z,
					 showscale=False,
					 opacity=1)
...
one = meshfile('Tool_Stand.stl',-552.66,-77.19,19.05-20,75.18-90,1)
...
ax2 = axesrgb1(-441.77,-214.48,534.24,-90)
for i in range(len(ax2)):
	graphs.append(ax2[i])

ax3 = axesrgb1(-399.59,-55.1,534.02,-90)
for i in range(len(ax3)):
	graphs.append(ax3[i])

fig = go.Figure(data=graphs, layout=layout)

fig.data[0].update(lighting=dict(ambient= 0.48,
								 diffuse= 1,
								 fresnel=  .1,
								 specular= 1,
								 roughness= .1,
...
fig.add_trace(go.Scatter3d(x=[0,-441.77], y=[0,-214.48], z=[20,534.24], name = 'Low 2014',marker=dict(color='black', size=5),line=dict(color='black', width=9, dash='dash')))
fig.add_trace(go.Scatter3d(x=[0,-399.59], y=[0,-55.1], z=[20,534.02], name = 'Low 2014',marker=dict(color='black', size=5),line=dict(color='black', width=9, dash='dash')))
fig.add_trace(go.Scatter3d(x=[-441.77,-399.59], y=[-214.48,-55.1], z=[534.24,534.02], name = 'Low 2014',marker=dict(color='orange', size=1),line=dict(color='orange', width=9)))
