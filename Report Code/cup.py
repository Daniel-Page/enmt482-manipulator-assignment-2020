...
def cup_tool_entry_attach():
	# The entry coordinates of the cup tool in the base reference frame
	base_P_cup_tool_entry = np.matrix([-358.13, 102.68, 532.67]).T

	# Portafilter entry in the base reference frame
	base_P_grinder = np.matrix([-399.59, -55.1, 534.02]).T

	# Grinder tool entry in the base reference frame
	base_P_adpnt = np.matrix([-441.77, -214.48, 534.24]).T
	...
	base_T_cup_tool_entry = rdk.TxyzRxyz_2_Pose([base_P_cup_tool_entry[0], base_P_cup_tool_entry[1], base_P_cup_tool_entry[2], 0, 0, theta])
	cup_tool_entry_T_rotations = rdk.TxyzRxyz_2_Pose([0, 0, 0, 0, -np.pi, np.radians(50)+np.pi+np.pi/2])
	
	return base_T_cup_tool_entry*cup_tool_entry_T_rotations


def cup_tool_entry_detach():
	# The entry coordinates of the cup tool in the base reference frame
	base_P_cup_tool_entry = np.matrix([-358.13, 102.68, 532.67]).T
	...
	base_T_cup_tool_entry = rdk.TxyzRxyz_2_Pose([base_P_cup_tool_entry[0], base_P_cup_tool_entry[1], base_P_cup_tool_entry[2], 0, 0, theta])
	cup_tool_entry_T_rotations = rdk.TxyzRxyz_2_Pose([0, 0, 0, 0, -np.pi, np.radians(50)])
...
def portafilter_handover_to_cup_tool(robot):
	...
	[-167.33871430362592, -69.46941136533248, -83.76680828259695, -116.83114037300633, 91.65220493549874, -192.40639086933095]]
	
	for pos in servo_positions:
		robot.MoveJ(pos)

	# Approach the cup tool on the stand
	base_T_cup_tool_approach = cup_tool_entry_attach()
	
	robot.MoveL(base_T_cup_tool_approach)
...
def lift_cup_from_stack(robot):
	...
	# Move cup upwards off the stack
