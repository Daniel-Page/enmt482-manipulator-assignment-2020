...
def portafilter_entry():
	# Portafilter entry point in the base reference frame
	base_P_portafilter_entry = np.matrix([-399.59, -55.1, 534.02]).T

	# Grinder tool entry point in the base reference frame
	base_P_grinder_tool_entry = np.matrix([-441.77, -214.48, 534.24]).T

	# The change in coordinates with respect to the base
	dspmt_vector = base_P_grinder_tool_entry - base_P_portafilter_entry

	# Determine the angle from the x axis of the displacement vector
	dspmt_xy_vector = [float(dspmt_vector[0]), float(dspmt_vector[1])] # The displacement in terms of x and y

	theta = np.arctan(dspmt_xy_vector[1]/dspmt_xy_vector[0])

	base_T_portafilter_entry = rdk.TxyzRxyz_2_Pose([base_P_portafilter_entry[0], base_P_portafilter_entry[1], base_P_portafilter_entry[2], 0, 0, theta])
	portafilter_entry_T_rotations = rdk.TxyzRxyz_2_Pose([0, 0, 0, 0, -np.pi, np.radians(50)+np.pi+np.pi/2])
	
	return base_T_portafilter_entry*portafilter_entry_T_rotations


def grinder_tool_entry():
	# Grinder tool entry point in the base reference frame
	base_P_portafilter_entry = np.matrix([-399.59, -55.1, 534.02]).T
	...
	base_T_grinder_tool_entry = rdk.TxyzRxyz_2_Pose([base_P_grinder_tool_entry[0], base_P_grinder_tool_entry[1], base_P_grinder_tool_entry[2], 0, 0, theta])
	grinder_tool_entry_T_rotations = rdk.TxyzRxyz_2_Pose([0, 0, 0, 0, -np.pi, np.radians(50)+np.pi+np.pi/2])
	return base_T_grinder_tool_entry*grinder_tool_entry_T_rotations
...
def home_to_tool_stand_portafilter(robot):
	...
	base_T_portafilter_tool_approach = portafilter_entry()
...
def detach_portafilter(robot, RDK, world_frame):
	...
	# Move back to provide clearance
...
def stand_to_tool_stand(robot):
	...
	[-133.8551160227819, -86.1107182316921, -71.06197222751628, -112.97661208050451, 89.96102046118116, -159.58587940692362]]

	for pos in servo_positions:
		robot.MoveJ(pos)

	# Approach the grinder tool on the stand
	base_T_grinder_tool_approach = grinder_tool_entry()
