...
# Right click on the provided programms and click 'run on robot' for them to work
# Open the connect menu from the toolbar and press connect
# Double click 'Disconnect' or press stop when finished
...
robot.MoveJ(target)                                       #  0. Reset the UR5 to the home position
...
grinder.crank_grinder_lever(robot, 62)                    # 11. Crank the lever [arg 2] degrees, then returns to its previous position
grinder.crank_grinder_lever(robot, 62)                    # 12. Crank the lever [arg 2] degrees, then returns to its previous position
grinder.crank_grinder_lever(robot, 62)                    # 13. Crank the lever [arg 2] degrees, then returns to its previous position
...
coffee_machine.tamper_to_coffee_machine(robot, 20)        # 21. Move to a position where the portafilter can be manually removed and connect to the coffee machine
...
coffee_machine.coffee_switch(robot, 12)                   # 37. Turn the coffee switch on and off with a delay between
...
coffee_machine.exit_cup_standoff(robot)                   # 46. Exit out of the stand-off of the coffee machine
coffee_machine.serve_cup(robot)                           # 47. Move the cup into a position where it can be taken
