...
from grinder import rotate_arm_T, grinder_tool_entry
from cup import cup_tool_entry_attach, cup_tool_entry_detach
...
def base_T_coffmch():
	...
	theta = np.arctan2(-dspmt_xy_vector[0], dspmt_xy_vector[1])
...
def place_cup_in_coffmch(robot):
	...
	# Frames
	x_offset = 8 # To esnure that the tool does not hit the back wall
...
def exit_cup_standoff(robot):
	...
	# Exit stand-off with constant height
	coffmch_T_base = base_T_coffmch()*coffmch_T_cup_stand*top_of_cup*cup_tool_center_T_tcp*rotate_arm_T()
...
def stand_off_to_tool_stand(robot):
	...
	T_cup_tool_approach = cup_tool_entry_detach()
...
def coffee_switch(robot, delay):
	...
	# Transformations
	base_T_on_apprch = base_T_coffmch()*coffmch_T_coffee_switch*pointer_T_end_tcp*coffee_switch_T_off_apprch*rotate_arm_T()
	# base_T_on = base_T_coffmch()*coffmch_T_coffee_switch*pointer_T_end_tcp*coffee_switch_T_off*rotate_arm_T()
	base_T_off_apprch = base_T_coffmch()*coffmch_T_coffee_switch*pointer_T_end_tcp*coffee_switch_T_on_apprch*rotate_arm_T()
	# base_T_off = base_T_coffmch()*coffmch_T_coffee_switch*pointer_T_end_tcp*coffee_switch_T_on*rotate_arm_T()

	# Switch on, delay, then switch off
...
def coffee_switch_to_stand(robot):
	...
	# Tool stand grinder tool entry position 
	base_T_grinder_tool_approach = grinder_tool_entry()
...
def approach_cup_tool_grinder(robot):
	...
	T_cup_tool_approach = cup_tool_entry_attach()
...
def approach_stand_off(robot):
	...
	# Move tool towards cup in stand-off
	coffmch_T_base = base_T_coffmch()*coffmch_T_cup_stand*top_of_cup*cup_tool_center_T_tcp*rotate_arm_T()
